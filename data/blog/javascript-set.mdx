...
In a previous article, we discussed [Javascript Map](https://blog.imam.dev/blog/javascript-hashmap). Set is similar to Map, but it only stores the keys, not the values. It's a great way to keep unique values of any type. It's overwhelming to use Map in so many cases. That's why let's talk about Set.
...
{/* This article is originally published on [blog.imam.dev](https://blog.imam.dev/blog/javascript-set). */}
